...
public function showTvShowsPage() {
    ...
    // Get the logged-in user's ID, if it exists
    $userId = $_SESSION['user_id'] ?? null;

    // Loop through each TV show to add extra data
    foreach ($tvShows as &$show) {
        $show['genres'] = $genreModel->findAllByTvShowId($show['id']);
        $show['number_of_seasons'] = $tvShowModel->getSeasonCount($show['id']);
        
        // --- THE FIX IS HERE ---
        // 1. Check if this specific show is in the user's watchlist
        $show['isInWatchlist'] = false; // Default to false
        if ($userId) {
            $show['isInWatchlist'] = $watchlistModel->isInWatchlist($userId, 'tv_show', $show['id']);
        }

        // 2. Create the watch URL for each TV show
    ...
    unset($show); // Break the reference after the loop

    return view('list-all-tv-shows', [
        'title' => 'TV Shows',
        'items' => $tvShows, // The $tvShows array now contains all the necessary data for each show
        'pageTitle' => 'All TV Shows'
        // We no longer need to pass a separate 'isInWatchlist' variable
    ]);
}
    ...
    public function showMovieDetail($slug) { // Artık $id değil, $slug alıyor
        ...
        $isInWatchlist = $watchlistModel->isInWatchlist($_SESSION['user_id'], 'movie', $movie['id']);
        }

        $isInWatchlist = false;
        if ($userId) {
        $isInWatchlist = $watchlistModel->isInWatchlist($userId, 'movie', $movie['id']);
        }
        
        return view('movie-detail', [
            'title' => $movie['title'],
            'movie' => $movie,
            'isInWatchlist' => $isInWatchlist // Pass this to the view
        ]);
...
// Add this new function inside your HomeController class, before the final '}'
public function toggleWatchlist() {
    if (session_status() === PHP_SESSION_NONE) { session_start(); }

    if (!isset($_SESSION['user_id'])) {
        header('Content-Type: application/json');
        echo json_encode(['success' => false, 'status' => 'error', 'message' => 'Login required.']);
        exit();
    }
    ...
    $watchlistModel = new \App\Models\WatchlistModel();
    $status = $watchlistModel->toggleStatus($userId, $contentType, $contentId);

    header('Content-Type: application/json');
    echo json_encode(['success' => true, 'status' => $status]); // Returns 'added' or 'removed'
    exit();
}
