...
<div class="hero-section" style="background-image: url('https://image.tmdb.org/t/p/original<?php echo $tvShow['backdrop_path']; ?>');">
    ...
    <div class="main-buttons">
        ...
        <?php endif; ?>
                <button class="btn btn-watchlist" data-content-id="<?php echo $movie['id'] ?? $tvShow['id']; ?>" data-content-type="<?php echo isset($movie) ? 'movie' : 'tv_show'; ?>">
...
// Add this code to the top of your existing DOMContentLoaded listener

const watchlistBtn = document.querySelector('.btn-watchlist');

if (watchlistBtn) {
    // Set initial state from PHP variable
    let isInWatchlist = <?php echo json_encode($isInWatchlist ?? false); ?>;
    
    function updateButtonState(isAdded) {
        ...
        if (isAdded) {
            ...
            text.textContent = 'Add Watchlist';
        }
    }
    ...
    watchlistBtn.addEventListener('click', function() {
        ...
        if (!isInWatchlist && !<?php echo isset($_SESSION['user_id']) ? 'true' : 'false'; ?>) {
        ...
        .then(data => {
            if (data.success) {
                isInWatchlist = data.status === 'added';
                updateButtonState(isInWatchlist);
            } else {
                alert(data.message || 'An error occurred.');
            }
